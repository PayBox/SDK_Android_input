    ...
    protected void onHandleIntent(Intent intent) {
        if(intent.hasExtra(RESPRECIEVER_EXTRA)&&intent.hasExtra(CONFIG_EXTRA)&&intent.hasExtra(COMMAND_EXTRA)&&intent.hasExtra(SECRET_EXTRA)){
            ...
            HashMap<String, String> param = parser.sort((HashMap<String, String>) intent.getExtras().get(CONFIG_EXTRA));
                switch (operation){
                    case PAYMENT:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_ENTRY_URL,param));
                        initPostRequest(Constants.PB_MAIN+Constants.PB_ENTRY_URL, param);
                        break;
                    case GETSTATUS:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_STATUS_URL,param));
                        initPostRequest(Constants.PB_MAIN+Constants.PB_STATUS_URL, param);
                        break;
                    case REVOKE:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_REVOKE_URL,param));
                        initPostRequest(Constants.PB_MAIN+Constants.PB_REVOKE_URL, param);
                        break;
                    case CANCEL:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_CANCEL_URL,param));
                        initPostRequest(Constants.PB_MAIN+Constants.PB_CANCEL_URL, param);
                        break;
                    case RECURRING:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_RECURRING_URL,param));
                        initPostRequest(Constants.PB_MAIN+Constants.PB_RECURRING_URL, param);
                        break;
                    case CARDADD:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_ADDCARD_URL,param));
                        initPostRequest(Constants.PB_CARD_MERCHANT(param.get(Constants.MERCHANT_ID)).concat(Constants.PB_ADDCARD_URL), param);
                        break;
                    case CARDLIST:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_LISTCARD_URL,param));
                        initPostRequest(Constants.PB_CARD_MERCHANT(param.get(Constants.MERCHANT_ID)).concat(Constants.PB_LISTCARD_URL), param);
                        break;
                    case CARDREMOVE:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_REMOVECARD_URL,param));
                        initPostRequest(Constants.PB_CARD_MERCHANT(param.get(Constants.MERCHANT_ID)).concat(Constants.PB_REMOVECARD_URL), param);
                        break;
                    case CARDPAYINIT:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_CARDINITPAY,param));
                        initPostRequest(Constants.PB_CARDPAY_MERCHANT(param.get(Constants.MERCHANT_ID)).concat(Constants.PB_CARDINITPAY), param);
                        break;
                    case CAPTURE:
                        param.put(Constants.SIG, parser.getSig(intent.getStringExtra(SECRET_EXTRA),Constants.PB_DO_CAPTURE_URL,param));
